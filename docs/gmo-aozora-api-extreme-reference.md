...
この章は、似たように見える ID 群を役割ごとに整理したものです。  
既存の `重要ID` 章より一歩踏み込んで、「どの ID 同士をどう結びつけるか」を扱います。

## 47.1 `applyNo` と `acceptNo`

| 項目 | 役割 | どこで出るか |
| --- | --- | --- |
| `applyNo` | 申請単位の受付番号 | `TransferRequestResponse`, `TransferRequestResultResponse`, `TransferApply` |
| `acceptNo` | 銀行受付単位の番号 | `TransferAccept` |

実装上の理解:

- `applyNo` は「クライアントが追跡する主キー」
- `acceptNo` は「銀行側で成立/予約中になった受付の追跡キー」

業務ログの設計では `applyNo` を主に使い、`acceptNo` は補助キーとして保持するとよいです。

## 47.2 `itemId` と `itemKey`
...
- `itemId` は送信時にクライアントが付ける
- `itemKey` は明細照会時に返る銀行側キー

したがって、送信リクエストと照会レスポンスの照合では、
...
つまり、`raId -> many vaId` の関係を前提にすると設計しやすいです。

## 47.4 `messageId`

`messageId` は Webhook メッセージを一意に識別する ID です。  
通知配信と未送信回収APIの両方で重複排除キーとして扱うのが最も自然です。
...
| 振込申請 | `applyNo` |
| 振込受付 | `acceptNo` |
| 入出金明細 | `accountId + itemKey` |
| 仮想口座入金明細 | `vaId + itemKey` |
| Webhook | `messageId` |

## 48. 同期・再取得戦略

この章は REST API と Webhook をどう組み合わせるかの実装指針です。  
既存の STG 疎通試験や Webhook 説明とは別に、「本番運用でズレをどうなくすか」に絞っています。
...
| 残高 | オンデマンド照会 |
| 明細 | 差分または対象期間指定で再取得 |
| 振込状態 | `applyNo` ベースでポーリング |
| 仮想口座一覧 | 条件付き一覧 + ページング追跡 |

## 48.2 Webhook と REST の役割分担
...
つまり、Webhook 単独ではなく REST との二段構えが前提です。

## 48.3 取りこぼしに強いパターン

おすすめの流れ:

1. Webhook を受信したら `messageId` で保存
2. 定期的に `GET /unsentlist/va-deposit-transaction` で補完
3. さらに必要なら `GET /va/deposit-transactions` で期間再同期

この三段構えにすると、通知漏れ・一時障害・再配信のすべてに強くなります。
...
1. `request` 応答を保存
2. `resultCode=2` なら `request-result` を追う
3. 最終的に `status` を正として保存

つまり、更新系の source of truth は最終的に `status` 側へ寄せるのが自然です。
...
この章は API の機能説明ではなく、本番運用の可観測性に関する整理です。
...
| API 呼び出し回数 | 利用量把握 |
| HTTP ステータス別件数 | 異常検知 |
| `resultCode=2` の滞留件数 | 未完了監視 |
| Webhook 受信件数 | 通知系監視 |
| 未送信取得件数 | 通知漏れ監視 |
| `hasNext=true` 継続回数 | ページング異常検知 |
...
| 呼び出しAPI名 | 追跡 |
| 実行時刻 | 時系列分析 |
| `accountId` / `applyNo` / `vaId` | 事象特定 |
| `resultCode` | 更新系状態確認 |
| `hasNext` / `nextItemKey` | 差分同期検証 |
| `messageId` | Webhook重複排除 |

## 49.3 アラート候補

| 条件 | 意味 |
| --- | --- |
| `request-result` が長時間 `2` のまま | 処理滞留 |
| Webhook 受信ゼロが長時間続く | 配信停止や経路障害 |
| 未送信取得件数が急増 | 通知失敗 |
| `400` が急増 | 入力バリデーション不整合 |
| `401/403` が急増 | 認証・認可問題 |

## 50. 推論ベースの状態遷移メモ

この章は公式の状態遷移図そのものではありません。  
公開されている状態名から実装上ほぼそう扱うべき、という推論メモです。

## 50.1 `transferStatus` の推論
...
申請中 -> 取下げ
申請中/予約中 -> 承認取消/予約取消
手続中 -> リトライ中 -> 手続済 or 手続不成立
手続済 -> 資金返却 or 組戻系状態
```

実装上の使い道:

- 画面で「進行中」「完了」「失敗」「要対応」に丸める
- MCPツールで自然言語説明を簡潔に返す

## 50.2 `applyStatus` の推論

推論上の大まかな流れ:

```text
未申請 -> 申請中 -> 承認済 -> 自動承認 or 実行へ
申請中 -> 差戻
申請中 -> 取下げ
申請中 -> 期限切れ
承認済 -> 承認取消
...
重要なのは、`applyStatus` は申請ワークフロー、`transferStatus` は資金移動ワークフローだという点です。

## 50.3 UI/LLM向けの丸めカテゴリ

| 生状態 | 丸め表示 |
| --- | --- |
| `申請中`, `予約中`, `手続中`, `リトライ中` | 進行中 |
| `手続済` | 完了 |
| `差戻`, `取下げ`, `承認取消`, `期限切れ` | 中断 |
| `手続不成立`, `不能・組戻あり` | 失敗 |
| `資金返却`, `組戻*` | 要後続確認 |
...
この章は公開リポジトリで実装を進めるためのテストデータ戦略です。  
AGENTS.md の漏洩防止ルールを実装レベルに落としたものとして読むと使いやすいです。
...
| 層 | 目的 |
| --- | --- |
| 認証モック | OAuth フローの疎通確認 |
| 口座モック | 一覧/残高表示 |
| 明細モック | ページングと差分同期 |
| 振込モック | 申請、未完了、完了、失敗 |
| Webhookモック | 即時受信、重複、未送信回収 |

## 51.3 最低限ほしい異常系フィクスチャ

- `resultCode=2`
- `resultCode=8`
- `hasNext=true`
- `nextItemKey=null`
- `transferErrorDetails` あり
- `UnableDetailInfo` あり
- Webhook `404`
- `spAccounts` 省略

こうした異常系を先に持っておくと、見かけ上正常な実装で止まらずに済みます。

## 52. リクエスト/レスポンス対応表

この章は、API を呼ぶ側が「何を送ると、即時には何が返り、後で何を見れば最終状態が分かるか」を一目で引けるようにしたものです。

## 52.1 認証系

| API | リクエスト | 即時レスポンス | 後続で見るもの |
| --- | --- | --- | --- |
| `/authorization` | クエリパラメータ | リダイレクト | `code`, `state` |
| `/token` | `TokenRequest` | `TokenResponse` | 必要に応じて refresh |

## 52.2 口座系

| API | リクエスト | レスポンス | 後続 |
| --- | --- | --- | --- |
| `/accounts` | なし | `AccountsResponse` | 必要なら `balances` |
| `/accounts/balances` | なし or `accountId` | `BalancesResponse` | 再照会 |
| `/accounts/transactions` | `accountId`, 期間 | `TransactionsResponse` | `nextItemKey` 追従 |
| `/accounts/deposit-transactions` | `accountId`, 期間 | `DepositTransactionsResponse` | `nextItemKey` 追従 |
| `/accounts/visa-transactions` | `accountId`, 期間 | `VisaTransactionsResponse` | `nextItemKey` 追従 |

## 52.3 振込系

| API | リクエスト | 即時レスポンス | 最終確認 |
| --- | --- | --- | --- |
| `/transfer/transferfee` | `TransferRequest` | `TransferFeeResponse` | なし |
| `/transfer/request` | `TransferRequest` | `TransferRequestResponse` | `/transfer/request-result`, `/transfer/status` |
| `/transfer/request-result` | `accountId`, `applyNo` | `TransferRequestResultResponse` | `/transfer/status` |
| `/transfer/status` | `applyNo` or 条件 | `TransferStatusResponse` | これを正と扱う |
| `/transfer/cancel` | `TransferCancelRequest` | `TransferCancelResponse` | `/transfer/request-result`, `/transfer/status` |

## 52.4 総合振込系

| API | リクエスト | 即時レスポンス | 最終確認 |
| --- | --- | --- | --- |
| `/bulktransfer/transferfee` | `BulkTransferRequest` | `TransferFeeResponse` | なし |
| `/bulktransfer/request` | `BulkTransferRequest` | `BulkTransferRequestResponse` | `/bulktransfer/status` |
| `/bulktransfer/status` | `applyNo` or 条件 | `BulkTransferStatusResponse` | これを正と扱う |

## 52.5 仮想口座系

| API | リクエスト | 即時レスポンス | 後続 |
| --- | --- | --- | --- |
| `/va/issue` | `VaIssueRequest` | `VaIssueResponse` | `/va/list` |
| `/va/list` | `VaListRequest` | `VaListResponse` | `nextItemKey` 追従 |
| `/va/status-change` | `VaStatusChangeRequest` | `VaStatusChangeResponse` | `/va/list` |
| `/va/close-request` | `VaCloseRequest` | `VaCloseRequestResponse` | 契約側確認 |
| `/va/deposit-transactions` | `raId`/`vaId`, 期間 | `VaDepositTransactionsResponse` | `nextItemKey` 追従 |

## 52.6 つかいわけ口座間振替

| API | リクエスト | 即時レスポンス | 後続 |
| --- | --- | --- | --- |
| `/transfer/spaccounts-transfer` | `SpAccountTransferRequest` | `SpAccountTransferResponse` | 必要なら残高再照会 |

## 52.7 Webhook

| API | リクエスト | レスポンス | 後続 |
| --- | --- | --- | --- |
| `/subscribe` | `SubscribeRequestBody` | 空レスポンス | 配信状態監視 |
| `/unsentlist/va-deposit-transaction` | なし | `VaDepositTransactionUnsentResponse` | `404` まで継続 |

## 53. 永続化スキーマ設計の叩き台

この章は「もしアプリや MCP サーバーが状態を持つなら、何をどう保存するか」の設計メモです。  
API 理解そのものではなく、理解した仕様を保存モデルに落とし込むための補助線です。
...
| テーブル | キー | 主なカラム |
| --- | --- | --- |
| `accounts` | `account_id` | `account_type_code`, `account_number`, `account_name`, `currency_code`, `primary_account_code` |
| `sp_accounts` | `account_id` | `sp_account_type_code`, `sp_account_name`, `sp_account_branch_code`, `sp_account_number` |
...
| テーブル | 主キー候補 |
| --- | --- |
| `account_transactions` | `account_id + item_key` |
| `deposit_transactions` | `account_id + item_key` |
| `va_transactions` | `va_id + item_key` |
| `visa_transactions` | `account_id + use_date + approval_number + amount` |
...
- `transfer_requests`
- `transfer_status_snapshots`
- `transfer_accepts`
- `transfer_apply_details`
- `transfer_errors`
...
- `virtual_accounts`
- `virtual_account_status_history`
...
この章は、典型的な障害パターンごとに「次に何を見るか」を整理したものです。
...
1. 同内容を即再送しない
2. `applyNo` が取れていれば `request-result`
3. 取れていなければアプリ側送信履歴から照合
4. 可能なら `status` 側でも確認

## 54.2 `resultCode=2` のまま長時間変わらない

やること:

1. `request-result` を間隔を空けて再取得
2. `status` に出ていないか確認
3. 監視アラート対象にする

## 54.3 Webhook が急に来なくなった

やること:

1. `subscribe` 状態変更の履歴確認
2. 未送信取得 API を叩く
3. `404` か 200 かで切り分け
4. 期間指定の REST 照会で再同期
...
1. `hasNext` と `nextItemKey` の追跡漏れ確認
2. 同条件でのページング継続有無を確認
3. 期間を狭めて再照会

## 54.5 仮想口座状態変更後に表示が合わない

やること:

1. `VaStatusChangeResponse` を保存しているか確認
2. `va/list` の最新結果で上書き
3. 必要なら状態履歴テーブルへ snapshot を追加

## 54.6 つかいわけ口座間振替後に残高が合わない

やること:

1. `SpAccountTransferResponse` の `acceptDatetime` を記録
2. 対象口座の残高を再照会
3. つかいわけ口座と通常口座を取り違えていないか確認

## 55. MCP ツール出力設計

この章は、API をそのまま返すのではなく、LLM や人間が扱いやすい出力に整形するための指針です。
...
| ツール種別 | 返すべき要約 |
| --- | --- |
| 口座一覧 | 口座名、口座種別、通貨、`accountId` |
| 残高 | 現在残高、支払可能残高、基準日時 |
| 明細 | 取引日、金額、摘要、残高、`itemKey` |
| 仮想口座一覧 | `vaId`, 状態, 有効期限, 最新入金日 |
...
| ツール種別 | 返すべき要約 |
| --- | --- |
| 振込依頼 | `applyNo`, `resultCode`, 指定日, 合計金額 |
| 総合振込依頼 | `applyNo`, `resultCode`, 件数, 合計金額 |
| 仮想口座発行 | `vaTypeCode`, 発行件数, `vaList` の要約 |
| 状態変更 | 変更対象 `vaId` 一覧, 新状態 |
| つかいわけ口座間振替 | 出金元, 入金先, 金額, `acceptDatetime` |
...
振込依頼は受理されませんでした。
全体エラー: errorCode=...
失敗明細: itemId=2, errorDetailsCode=...
次に確認: 入力内容を修正して再見積、または status/request-result を照会
```
...
- 実口座番号は必要最小限だけ表示
- 可能なら名義はマスク
- `applyNo`, `vaId`, `messageId` は追跡キーとして返す

## 55.5 出力で優先すべき「人間が知りたいこと」

| 領域 | 優先表示 |
| --- | --- |
| 振込 | 成否、未完了か、受付番号、金額、次の確認先 |
| 仮想口座 | 発行/利用可能か、期限、入金有無 |
| Webhook | 新着か、重複か、回収由来か |
| 残高 | 現在残高、支払可能残高、基準時刻 |

## 56. デフォルト値・省略時挙動カタログ

この章は「省略したとき何になるか」をまとめたカタログです。  
既存章で各APIごとに散っている初期値や既定挙動を、実装判断に使える一覧として集約しています。

## 56.1 認証系の既定値

| 項目 | 既定/固定 | 意味 |
| --- | --- | --- |
| `response_type` | `code` 固定 | 認可コードフロー前提 |
| `offline_access` | scope に含めたときのみ有効 | refresh token の発行条件 |

## 56.2 振込指定日の既定挙動

| 項目 | 省略時/既定 | 備考 |
| --- | --- | --- |
| `transferDateHolidayCode` | `1` 扱い | 翌営業日 |
| `transferDesignatedDate` | 明示指定推奨 | 非営業日ロジックに影響 |

## 56.3 振込件数・金額の省略ルール

| モデル | 項目 | 省略条件 |
| --- | --- | --- |
| `TransferRequest` | `totalCount` | 1件のみなら省略可 |
| `TransferRequest` | `totalAmount` | 1件のみなら省略可 |
| `Transfer` | `itemId` | 1件のみなら省略可 |

## 56.4 総合振込照会の既定挙動

| 項目 | 省略時/既定 |
| --- | --- |
| `requestTransferClass` | `1` 扱い |
| `requestTransferTerm` | `1` 扱い |
| `detailInfoNecessity` | `false` 扱い |
| `bulktransferItemKey` | `1` 扱い。ただし `detailInfoNecessity=true` のとき |
...
| `requestTransferClass` | `1` 扱い |
| `requestTransferTerm` | `1` 扱い |
| `requestTransferStatuses` | 全対象扱い |

## 56.6 明細系の期間既定値

| API | `dateFrom/dateTo` 未指定時 |
| --- | --- |
| `deposit-transactions` | 当日扱い |
| `visa-transactions` | 当日扱い |
...
- サーバー既定値に依存しすぎない
- 内部正規化後の値を明示して保存する
- UI では「省略時に何になるか」を見せる

## 57. ページング実装アルゴリズム

この章は、ページングを単なる説明ではなくアルゴリズムとして固定するためのものです。

## 57.1 `nextItemKey` 型ページング

対象:

- `/accounts/transactions`
- `/accounts/deposit-transactions`
- `/accounts/visa-transactions`
...
1. 初回は `nextItemKey` なし
2. 応答の `hasNext` を確認
3. `hasNext=true` なら応答の `nextItemKey` を次回に渡す
4. それ以外の条件は一切変えない
5. `hasNext=false` で停止
...
- `/bulktransfer/status` で `detailInfoNecessity=true`

アルゴリズム:

1. `bulktransferItemKey=1` で開始
2. 500件ごとに次の開始位置を計算
3. `1 -> 501 -> 1001 ...` のように進める
4. 返却件数が500未満になったら停止候補
...
| API群 | 停止条件 |
| --- | --- |
| 一般ページング | `hasNext=false` |
| Webhook未送信回収 | `404` |
| 総合振込明細 | 返却件数不足または明細尽きたと判断できる状態 |

## 58. データ感度分類とマスキング規約

この章は、公開リポジトリ運用とアプリ内ログ設計を両立させるための区分です。

## 58.1 区分

| 区分 | 例 | ログ可否 |
| --- | --- | --- |
| `public-safe` | API path, resultCode, hasNext | 可 |
| `internal-sensitive` | accountId, applyNo, vaId, messageId | 条件付き可 |
| `restricted` | accountNumber, beneficiaryName, remitterName | 原則マスク |
| `secret` | access token, refresh token, client secret, webhook auth | 不可 |
...
| `vaId` | 追跡用に保存可 |
| `messageId` | 追跡用に保存可 |
| `accountNumber` | 表示時は末尾だけ |
| `beneficiaryName` | 画面では必要時のみ |
| `remitterName` | マスク推奨 |
| `accountName` | マスク推奨 |
| `x-access-token` | 保存禁止 |
| `refreshToken` | 保存禁止 |
...
- LLM に不要な実名義を渡さない
- 追跡に必要なキーだけ返す
- 後続照会に必要な `applyNo`, `vaId`, `messageId` を優先

## 59. 契約テスト優先順位

この章は「どの API から自動テストを固めるべきか」の順番です。  
既存の STG 疎通試験章は手動確認寄りなので、ここでは継続的な契約テストの優先順位として整理します。
...
- `/transfer/status`
- `/unsentlist/va-deposit-transaction` の `404` 終了条件
...
| 必須フィールド | `applyNo`, `resultCode`, `accountId` |
| optional の欠落 | `spAccounts`, `nextItemKey` |
| 型 | 文字列金額、Boolean フラグ |
| 既定値 | `detailInfoNecessity`, `currencyCode` |

## 60. ドキュメント/ソースのドリフト検知

この章は、公式ポータル・SDK docs・SDK source の差分にどう備えるかです。

## 60.1 典型的なドリフト

| 種類 | 例 |
| --- | --- |
| docs に出ない API | `spaccounts-transfer` |
| ホスト/パスの揺れ | `webhook` / `webhooks` |
| typo フィールド | `paymant*`, `transferDesignnatedType` |
| generated docs の表現差 | `No authorization required` |
...
- 既存章の焼き直しはしない
- 新しい差分や新APIだけを足す
- source と docs の差異は独立章で扱う

というルールが維持しやすいです。

## 61. データライフサイクルと保持期間設計

この章は、API から取得したデータをどの粒度で、どれくらい保持するかの設計指針です。  
公式仕様そのものではなく、このAPI群の性質から導かれる実装ガイドです。

## 61.1 保持対象の分類

| 分類 | 例 | 性質 |
| --- | --- | --- |
| マスタ | `accounts`, `spAccounts`, `VAccount` | 低頻度更新 |
| スナップショット | 残高、状態照会結果 | 時点依存 |
| イベント | 明細、Webhook message | 追記型 |
| ワークフロー | `applyNo`, `request-result`, `status` | 遷移型 |
| 秘密情報 | token, secret | 最短保持 |
...
- マスタ: 長めに保持し、更新で上書き
- スナップショット: 時系列比較が必要な期間だけ保持
- イベント: 重複排除できる範囲では長めに保持
- ワークフロー: 完了後も監査目的で保持
- 秘密情報: 必要最小限でローテーション前提

## 61.3 削除よりアーカイブを優先するもの

次は論理削除またはアーカイブ向きです。

- `applyNo` を起点にした振込ワークフロー
- `messageId` を持つ Webhook イベント
- `itemKey` を持つ明細
...
ただし、状態変化監査が必要なら履歴テーブルを別で持つ方が安全です。

## 61.5 秘密情報の保持原則

- アクセストークン: 短期
- リフレッシュトークン: 暗号化前提
- `clientSecret`: 永続保存するなら厳重管理
- Webhook認証情報: アプリ設定層のみ

アプリログ、ジョブログ、テストフィクスチャには残さない方がよいです。

## 62. アンチパターン集

この章は「仕様を読んでいてもやりがちな失敗」をまとめたものです。

## 62.1 タイムアウト時に書き込みAPIを即再送する
...
## 62.2 `itemId` を永続キーにしてしまう

`itemId` は送信時の明細番号です。  
永続的な照合キーとしては `itemKey` や `applyNo` ほど強くありません。
...
- `dateFrom` を途中で変える
- `sortOrderCode` を変える
- `requestTransferStatuses` を変える

これは取りこぼしや重複の原因になります。

## 62.4 `resultCode=1` だけ見て完了と決め打ちする

`request` 応答は受付寄りです。  
最終状態の確認は `status` 側まで見た方が安全です。

## 62.5 Webhook だけで正本化する

Webhook は即時性に強いですが、取りこぼし・再送・停止の問題があります。  
REST 照会と未送信回収を持たない設計は脆いです。

## 62.6 generated docs の文字列型をそのままドメイン型にする

金額・件数・日付が文字列で返るからといって、アプリ内部まで全部文字列にすると後で辛くなります。  
正規化層は用意した方がよいです。

## 62.7 optional フィールドの欠落を例外扱いする
...
が自然に出てきます。  
厳しすぎるデシリアライズは本番で壊れやすいです。

## 62.8 typo フィールドを勝手に修正して送信する

外部APIとの JSON マッピングまで変えると壊れます。  
内部正規化名を作るなら、外部境界で正確に変換する必要があります。

## 63. ペルソナ別 API 利用シナリオ

この章は、利用者ごとにどのAPIが主役になるかを整理したものです。
...
この章は「実装できたと言える条件」をテンプレート化したものです。  
既存のチェックリストよりもう少しテスト仕様寄りです。

## 64.1 口座一覧

受け入れ条件例:

- `accounts` が 0 件でも正常表示できる
- `spAccounts` 欠落でも落ちない
- `accountTypeCode` ごとに表示が崩れない

## 64.2 残高照会

受け入れ条件例:

- `withdrawableAmount` と `balance` を区別して表示できる
- 外貨口座で `fcyTotalBalance` と `yenEquivalent` が扱える
- `spAccountBalances` 欠落でも落ちない

## 64.3 明細照会

受け入れ条件例:

- `hasNext=true` のとき追続きを取得できる
- `nextItemKey=null` でも正常終了できる
- 空配列を正常系として扱える
...
- 500件を超える明細取得を分割できる
- `detailInfoNecessity` の分岐を実装している
- `bulktransferItemKey` の進め方が正しい
...
- 期限型と継続型の違いを扱える
- `va/list` の複数フィルタを同時に扱える
- 状態変更後に `va/list` で再整合できる
...
- `messageId` で重複排除できる
- 未送信回収 API の `404` を正常終了と扱える
- push と recovery の両経路を同一保存基盤に流し込める

## 64.8 つかいわけ口座間振替

受け入れ条件例:

- `currencyCode` 省略時を扱える
- 即時応答の `acceptDatetime` を保存できる
- 振替後の残高再照会ができる
